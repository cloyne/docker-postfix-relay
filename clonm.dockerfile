...
RUN apt-get update -q -q && \
 ...
 sed -i 's/smtpd_tls_cert_file=.*/smtpd_tls_cert_file=\/ssl\/live\/mail.cloyne.org\/fullchain.pem/' && \
 sed -i 's/smtpd_tls_key_file=.*/smtpd_tls_key_file=\/ssl\/live\/mail.cloyne.org\/privkey.pem' &&\
 sed -i 's/smtpd_tls_session_cache_database=.*/smtpd_tls_session_cache_database=btree:\/var\/run\/postfix\/smtpd_cache'
